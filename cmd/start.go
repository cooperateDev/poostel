...
func runMasterNodOnColdHot(ctx context.Context, config *configs.Config) error {
	...
	fileName, err := utils.CreateFile(ctx, filepath.Join(config.WorkingDir, "supernode.yml"), true)
	if err != nil {
		return err
	}
	...
	// Populate pastel.conf line-by-line to file.
	_, err = file.WriteString(fmt.Sprintf(configs.SupernodeDefaultConfig, pastelid, extIP, fmt.Sprintf("%d", flagMasterNodeRPCPort))) // creates server line
	if err != nil {
		return err
	}
	...
	remoteSuperNodePath = filepath.Join(remoteSuperNodePath, "supernode")

	log.WithContext(ctx).Info(fmt.Sprintf("RemoteSuperNodePath:%s", remoteSuperNodePath))

	_, err = RunCMD("scp", fileName, fmt.Sprintf("%s@%s:/%s/", username, flagMasterNodeSSHIP, remoteSuperNodePath))
	...
	go client.Cmd(fmt.Sprintf("%s %s", filepath.Join(config.PastelExecDir, constants.PastelSuperNodeExecName[utils.GetOS()]), fmt.Sprintf("--config-file=%s", filepath.Join(config.WorkingDir, "supernode", "supernode.yml")))).Run()
